...
class ControlsToMotors:
  def __init__(self):
    ...
    self.rover_on = rospy.get_param('~rover_on',False)
    if self.rover_on:
      #import gopigo
      #import atexit
      #atexit.register(gopigo.stop)
      import move
      self.rovermove = move.RoverMove()
  ...
  def motorcmd_2_robot(self, wheel='left', motor_command=0):
    if self.rover_on:
      ...
      #import gopigo
      if wheel == 'left':
        if motor_command >= 0: self.rovermove.moveL(1,motor_command_raw) #gopigo.motor1(1,motor_command_raw)
        elif motor_command < 0: self.rovermove.moveL(0,motor_command_raw) #gopigo.motor1(0,motor_command_raw)
      if wheel == 'right':
        if motor_command >= 0: self.rovermove.moveR(1,motor_command_raw) #gopigo.motor2(1,motor_command_raw)
        elif motor_command < 0: self.rovermove.moveR(0,motor_command_raw) #gopigo.motor2(0,motor_command_raw)
